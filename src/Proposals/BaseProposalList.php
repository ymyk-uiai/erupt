...
abstract class BaseProposalList extends BaseList implements Accessor
{
    use HandleAccess,
        HandleInitialize;

    protected static array $accessKeys = ['prs', 'proposals', 'proposalList'];
    ...
    {
        return "Erupt\\Proposals\\Lists\\".ucfirst($className);
    }
    ...
    public function extend(Ini $ini, string $descs, $scope = null): self
    {
        return array_reduce($this->split($descs), function ($carry, $desc) use ($ini, $scope) {
            ...
            $carry->add(BaseProposal::buildWithItsName($ini, $name, $args, $scope));
            return $carry;
        }, $this);
    }
    ...
    public function add(self|BaseProposal $incoming): void
    {
        $this->addItemOrList($incoming);
    }

    public function remove(self|BaseProposal $incoming): void
    {
